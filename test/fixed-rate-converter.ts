...
describe("FixedRateConverter", () => {
  ...
  const conversionFeeRate = BigNumber.from(4723550439442834); // 0.004723550439442834
  const initialSovAmount = toWei("1000000");
  const initialMyntAmount = toWei("10000000000");
  ...
  describe("convert", async () => {
    context("should success", async () => {
      it("convertAmount should return correct value", async () => {
        ...
        expect(convertedSovAmount.toString()).to.equal(
          BigNumber.from(myntAmount.toString()).mul(conversionFeeRate).div(precision),
        );
      });
    ...
    context("should revert", async () => {
      ...
      it("should revert if try to convert 0 mynt", async () => {
        ...
        await myntToken.approve(fixedRateConverter.address, myntToConvert);
        await expect(fixedRateConverter.connect(sender).convert(myntToConvert)).to.be.revertedWith(
      ...
      it("should revert if try to convert mynt amount > the sender balance", async () => {
        ...
        await myntToken.approve(fixedRateConverter.address, myntToConvert);
        await expect(fixedRateConverter.connect(sender).convert(myntToConvert)).to.be.revertedWith(
          "Error: amount exceeds MYNT balance",
        );
      });
    });
  });
